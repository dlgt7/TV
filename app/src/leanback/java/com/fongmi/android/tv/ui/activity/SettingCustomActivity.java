...
public class SettingCustomActivity extends BaseActivity implements MenuKeyDialog.Listener, X5WebViewDialog.Listener, LanguageDialog.Listener, CacheDirDialog.Listener {
    ...
    protected void initView(Bundle savedInstanceState) {
        ...
        mBinding.removeAdText.setText(getSwitch(Setting.isAdblock()));
    ...
    private void setSize(View view) {
        int index = PlayerSetting.getSize();
        PlayerSetting.putSize(index = index == size.length - 1 ? 0 : ++index);
    ...
    private String getSpeedText() {
        return String.format(Locale.getDefault(), "%.2f", SpeedSetting.getPlayback());
    }

    private void setSpeed(View view) {
        float speed = SpeedSetting.getPlayback();
    ...
    private void setCacheDir(View view) {
        PermissionX.init(this).permissions(Manifest.permission.WRITE_EXTERNAL_STORAGE).request((allGranted, grantedList, deniedList) -> {
            if (allGranted) CacheDirDialog.create(this).show();
        });
    }
