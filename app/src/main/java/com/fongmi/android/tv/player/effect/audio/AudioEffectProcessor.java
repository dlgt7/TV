...
public final class AudioEffectProcessor extends BaseAudioProcessor {
    ...
    private void applyStereo(float[] samples) {
        setStereo(samples, mixStereo(samples, true), mixStereo(samples, false));
    }
    ...
    private void applyMono(float[] samples) {
        float mono = sanitize((mixStereo(samples, true) + mixStereo(samples, false)) * 0.5f);
        samples[0] = mono;
        samples[1] = mono;
        clearExtraChannels(samples);
    }
    ...
    private void applyReverse(float[] samples) {
        float left = mixStereo(samples, true);
        float right = mixStereo(samples, false);
        setStereo(samples, right, left);
    }

    /** Public PCM down-mix used instead of the upstream private MPV bridge helper. */
    private static float mixStereo(float[] samples, boolean left) {
        if (samples.length == 0) return 0.0f;
        float value = samples[Math.min(left ? 0 : 1, samples.length - 1)];
        if (samples.length > 2) value += samples[2] * 0.7071f; // center
        if (samples.length > 3) value += samples[3] * 0.5f;    // LFE
        if (samples.length > 4) value += samples[left ? 4 : Math.min(5, samples.length - 1)] * 0.7071f;
        if (samples.length > 6) value += samples[left ? 6 : Math.min(7, samples.length - 1)] * 0.5f;
        return value;
    }
